...
PS_data <- read_csv("Data/PassaicRiver/2013 CPG CWCM Sampling - High Flow 1.csv")
...
PS_data <- PS_data %>%
  select(SAMPLE_NAME, SAMPLE_DATE, SAMPLE_TYPE_CODE, MATRIX_CODE, ANALYTIC_METHOD,
         Y_COORD, X_COORD, RESULT_UNIT, CHEMICAL_NAME, RESULT_NUMERIC)
...
# Create a new data frame with transposed values, treating duplicates as lists
transposed_data <- PS_data %>%
  pivot_wider(
    id_cols = c(SAMPLE_NAME, SAMPLE_DATE, SAMPLE_TYPE_CODE, MATRIX_CODE,
                ANALYTIC_METHOD, Y_COORD, X_COORD, RESULT_UNIT),
