...
results_rf_tPCB <- cbind(location = "New Bedford Harbor", results_rf_tPCB)
...
perform_grid_search <- function(congener_name, seed = 123) {
  ...
  congener_data <- nbh.pcb.1[[congener_name]]
  ...
  covariate_columns <- names(nbh.pcb.1)[!grepl("^PCB", names(nbh.pcb.1))]
  
  # Select rows with non-missing values in the specified PCB congener and extract covariates
  congener_covariate_data <- nbh.pcb.1[non_missing_indices, covariate_columns]
  ...
  results_rf_PCBi <- data.frame(
    Location = rep("New Bedford Harbor", length(test_indices)),
...
for (congener_name in colnames(nbh.pcb.1)) {
