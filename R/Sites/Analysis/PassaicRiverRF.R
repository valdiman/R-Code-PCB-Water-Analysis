...
results_rf_tPCB <- cbind(location = "Passaic River", results_rf_tPCB)
...
plotRF <- ggplot(results_rf_tPCB, aes(x = Observation, y = Predicted)) +
  ...
  annotate("text", x = 10^1.6, y = 10^5.5, label = "Passaic River", size = 12) +
...
{
  # Remove metadata
  pas.pcb <- subset(pas, select = -c(Source:AroclorCongener))
  # Remove Aroclor data
  ...
  # Change date format
  SampleDate <- as.Date(pas$SampleDate, format = "%Y-%m-%d")
  # Calculate sampling time
...
perform_grid_search <- function(congener_name, seed = 123) {
  ...
  congener_data <- pas.pcb.1[[congener_name]]
  ...
  covariate_columns <- names(pas.pcb.1)[!grepl("^PCB", names(pas.pcb.1))]
  
  # Select rows with non-missing values in the specified PCB congener and extract covariates
  congener_covariate_data <- pas.pcb.1[non_missing_indices, covariate_columns]
  ...
  hyperparameters <- expand.grid(
    num.trees = c(500, 750, 1000),  # Example values, adjust as needed
    mtry = c(3, 5, min(5, ncol(train_data))),  # Ensure mtry is less than or equal to the number of variables
    min.node.size = c(4, 5, 6)      # Example values, adjust as needed
  )
  ...
  results_rf_PCBi <- data.frame(
    Location = rep("Passaic River", length(test_indices)),
...
for (congener_name in colnames(pas.pcb.1)) {
