...
lme.spo.tpcb <- lmer(log10(tpcb) ~ 1 + time + poly(flow, 2) + season + (1|site),
                     REML = FALSE,
...
# Look at residuals
{
  ...
  # Create pdf file
  pdf("Output/Plots/Sites/Q-Q/Q-QSpokaneRiverQ-QtPCBQuad.pdf")
  qqnorm(res.spo.tpcb,
...
# Plot residuals vs. predictions
{
  ...
  png("Output/Plots/Sites/Residual/res_plotlmeSpokaneRivertPCBQuad.png",
...
shapiro.test(resid(lme.spo.tpcb)) # p-value = 0.1753
...
# Create matrix to store results
{
  ...
  lme.tpcb[7] <- fixef(lme.spo.tpcb)[3] # flow linear
  lme.tpcb[8] <- summary(lme.spo.tpcb)$coef[3,"Std. Error"] # flow error
  lme.tpcb[9] <- summary(lme.spo.tpcb)$coef[3,"Pr(>|t|)"] # flow p-value
  lme.tpcb[10] <- fixef(lme.spo.tpcb)[4] # flow quadratic
  lme.tpcb[11] <- summary(lme.spo.tpcb)$coef[4,"Std. Error"] # flow error
  lme.tpcb[12] <- summary(lme.spo.tpcb)$coef[4,"Pr(>|t|)"] # flow p-value
  lme.tpcb[13] <- fixef(lme.spo.tpcb)[5] # season 2
  lme.tpcb[14] <- summary(lme.spo.tpcb)$coef[5,"Std. Error"] # season 2 error
  lme.tpcb[15] <- summary(lme.spo.tpcb)$coef[5,"Pr(>|t|)"] # season 2 p-value
  lme.tpcb[16] <- fixef(lme.spo.tpcb)[6] # season 3
  lme.tpcb[17] <- summary(lme.spo.tpcb)$coef[6,"Std. Error"] # season 3 error
  lme.tpcb[18] <- summary(lme.spo.tpcb)$coef[6,"Pr(>|t|)"] # season 3 p-value
  lme.tpcb[19] <- -log(2)/lme.tpcb[4]/365 # t0.5
  lme.tpcb[20] <- abs(-log(2)/lme.tpcb[4]/365)*lme.tpcb[5]/abs(lme.tpcb[4]) # t0.5 error
  lme.tpcb[21] <- as.data.frame(VarCorr(lme.spo.tpcb))[1,'sdcor']
  lme.tpcb[22] <- as.data.frame(r.squaredGLMM(lme.spo.tpcb))[1, 'R2m']
  lme.tpcb[23] <- as.data.frame(r.squaredGLMM(lme.spo.tpcb))[1, 'R2c']
  lme.tpcb[24] <- shapiro.test(resid(lme.spo.tpcb))$p.value
  # Calculate RMSE
  # Predictions
...
# Create matrix to store results
lme.pcb <- matrix(nrow = length(spo.pcb.3[1,]), ncol = 25)

# Perform LME
for (i in 1:length(spo.pcb.3[1,])) {
  fit <- lmer(spo.pcb.3[,i] ~ 1 + time + poly(flow, 2) + season + (1|site),
              REML = FALSE,
              control = lmerControl(check.nobs.vs.nlev = "ignore",
                                    check.nobs.vs.rankZ = "ignore",
                                    check.nobs.vs.nRE="ignore"))
...
# Export results
write.csv(lme.pcb.t,
          file = "Output/Data/Sites/csv/SpokaneRiver/Quadratic/SpokaneRiverLmeQuadPCB.csv",
          row.names = FALSE)
