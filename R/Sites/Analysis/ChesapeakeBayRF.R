...
results_rf_tPCB <- cbind(location = "Chesapeake Bay", results_rf_tPCB)
...
plotRF <- ggplot(results_rf_tPCB, aes(x = Observation, y = Predicted)) +
  ...
  annotate("text", x = 10^2.95, y = 10^4.7, label = "Chesapeake Bay", size = 12) +
...
perform_grid_search <- function(congener_name, seed = 123) {
  ...
  congener_data <- che.pcb.2[[congener_name]]
  ...
  covariate_columns <- names(che.pcb.2)[!grepl("^PCB", names(che.pcb.2))]
  
  # Select rows with non-missing values in the specified PCB congener and extract covariates
  congener_covariate_data <- che.pcb.2[non_missing_indices, covariate_columns]
  ...
  results_rf_PCBi <- data.frame(
    Location = rep("Chesapeake Bay", length(test_indices)),
...
for (congener_name in colnames(che.pcb.2)) {
