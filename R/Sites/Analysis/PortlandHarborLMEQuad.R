...
lme.por.tpcb <- lmer(log10(tpcb) ~ 1 + time + poly(flow, 2) + tem + season + (1|site),
                     REML = FALSE,
...
# Look at residuals
{
  ...
  # Create pdf file
  pdf("Output/Plots/Sites/Q-Q/Q-QPortlandHarbortPCBQuad.pdf")
  qqnorm(res.por.tpcb,
...
# Plot residuals vs. predictions
{
  ...
  png("Output/Plots/Sites/Residual/res_plotlmePortlandHarbortPCBQuad.png",
...
# Shapiro test
shapiro.test(resid(lme.por.tpcb)) # p-value = 0.873

# Create matrix to store results from lme analysis
{
  lme.tpcb <- matrix(nrow = 1, ncol = 31)
  ...
  lme.tpcb[7] <- fixef(lme.por.tpcb)[3] # flow linear
  lme.tpcb[8] <- summary(lme.por.tpcb)$coef[3,"Std. Error"] # flow error
  lme.tpcb[9] <- summary(lme.por.tpcb)$coef[3,"Pr(>|t|)"] # flow p-value
  lme.tpcb[10] <- fixef(lme.por.tpcb)[4] # flow quadratic
  lme.tpcb[11] <- summary(lme.por.tpcb)$coef[4,"Std. Error"] # flow error
  lme.tpcb[12] <- summary(lme.por.tpcb)$coef[4,"Pr(>|t|)"] # flow p-value
  lme.tpcb[13] <- fixef(lme.por.tpcb)[5] # temperature
  lme.tpcb[14] <- summary(lme.por.tpcb)$coef[5,"Std. Error"] # temperature error
  lme.tpcb[15] <- summary(lme.por.tpcb)$coef[5,"Pr(>|t|)"] # temperature p-value
  lme.tpcb[16] <- fixef(lme.por.tpcb)[6] # season 1
  lme.tpcb[17] <- summary(lme.por.tpcb)$coef[6,"Std. Error"] # season 1 error
  lme.tpcb[18] <- summary(lme.por.tpcb)$coef[6,"Pr(>|t|)"] # season 1 p-value
  lme.tpcb[19] <- fixef(lme.por.tpcb)[7] # season 2
  lme.tpcb[20] <- summary(lme.por.tpcb)$coef[7,"Std. Error"] # season 2 error
  lme.tpcb[21] <- summary(lme.por.tpcb)$coef[7,"Pr(>|t|)"] # season 2 p-value
  lme.tpcb[22] <- fixef(lme.por.tpcb)[8] # season 3
  lme.tpcb[23] <- summary(lme.por.tpcb)$coef[8,"Std. Error"] # season 3 error
  lme.tpcb[24] <- summary(lme.por.tpcb)$coef[8,"Pr(>|t|)"] # season 3 p-value
  lme.tpcb[25] <- -log(2)/lme.tpcb[4]/365 # t0.5
  lme.tpcb[26] <- abs(-log(2)/lme.tpcb[4]/365)*lme.tpcb[5]/abs(lme.tpcb[4]) # t0.5 error
  lme.tpcb[27] <- as.data.frame(VarCorr(lme.por.tpcb))[1,'sdcor']
  lme.tpcb[28] <- as.data.frame(r.squaredGLMM(lme.por.tpcb))[1, 'R2m']
  lme.tpcb[29] <- as.data.frame(r.squaredGLMM(lme.por.tpcb))[1, 'R2c']
  lme.tpcb[30] <- shapiro.test(resid(lme.por.tpcb))$p.value
  # Calculate RMSE
  # Predictions
  ...
  lme.tpcb[31] <- sqrt(mean(residuals[non_na_indices]^2))
}
...
for (i in 1:length(por.pcb.3[1,])) {
  fit <- lmer(por.pcb.3[,i] ~ 1 + time + poly(flow, 2) + temp + season + (1|site),
              REML = FALSE,
              control = lmerControl(check.nobs.vs.nlev = "ignore",
                                    check.nobs.vs.rankZ = "ignore",
                                    check.nobs.vs.nRE="ignore"))
...
# Export results
write.csv(lme.pcb.t,
          file = "Output/Data/Sites/csv/PortlandHarbor/Quadratic/PortlandHarborLmeQuadPCB.csv",
          row.names = FALSE)
