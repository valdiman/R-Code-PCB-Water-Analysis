...
## Hudson River
## Only Linear Mixed-Effects Model (lme) w/ quadratic function for the flow
...
# tPCB vs. time + season + flow^2 + temp + site
lme.hud.tpcb.2 <- lmer(log10(tpcb) ~ 1 + time + poly(flow, 2) + wtemp + season + (1|site),
                      REML = FALSE,
...
# See results
summary(lme.hud.tpcb.2)

# Normality test
shapiro.test(resid(lme.hud.tpcb.2))$p.value # It doesn't work.
...
for (i in 1:length(hud.pcb.4[1,])) {
  fit <- lmer(hud.pcb.4[,i] ~ 1 + time + poly(flow, 2) + wtemp + season + (1|site),
              REML = FALSE,
              control = lmerControl(check.nobs.vs.nlev = "ignore",
                                    check.nobs.vs.rankZ = "ignore",
                                    check.nobs.vs.nRE="ignore"))
