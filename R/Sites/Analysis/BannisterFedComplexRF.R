...
results_rf_tPCB <- cbind(location = "Bannister Fed Complex", results_rf_tPCB)
...
perform_grid_search <- function(congener_name, seed = 123) {
  ...
  congener_data <- bfc.pcb.1[[congener_name]]
  ...
  covariate_columns <- names(bfc.pcb.1)[!grepl("^PCB", names(bfc.pcb.1))]
  
  # Select rows with non-missing values in the specified PCB congener and extract covariates
  congener_covariate_data <- bfc.pcb.1[non_missing_indices, covariate_columns]
  ...
  hyperparameters <- expand.grid(
    num.trees = c(100, 500, 1000),  # Example values, adjust as needed
  ...
  results_rf_PCBi <- data.frame(
    Location = rep("Bannister Fed Complex", length(test_indices)),
...
for (congener_name in colnames(bfc.pcb.1)) {
