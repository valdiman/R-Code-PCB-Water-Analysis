...
{
  # Change date format
  SampleDate <- as.Date(grl$SampleDate, format = "%Y-%m-%d")
...
results_rf_tPCB <- cbind(location = "Lake Michigan", results_rf_tPCB)

# Export results
write.csv(results_rf_tPCB,
          file = "Output/Data/Sites/csv/GreatLakes/LakeMichiganRFtPCB.csv",
          row.names = FALSE)
...
plotRF <- ggplot(results_rf_tPCB, aes(x = Observation, y = Predicted)) +
  ...
  annotate("text", x = 10^3.2, y = 10^1.2, label = "Lake Michigan", size = 12) +
...
ggsave("Output/Plots/Sites/ObsPred/GreatLakes/GreatLakesRFtPCB.png",
       plot = plotRF, width = 10, height = 5, dpi = 500)
# Random Forest doesn't work for tPCB, R2<0!

# Random Forest Model individual PCBs -------------------------------------
# Prepare data.frame
{
  grl.pcb <- subset(grl, select = -c(Source:AroclorCongener))
  # Remove Aroclor data
...
perform_grid_search <- function(congener_name, seed = 123) {
  ...
  congener_data <- grl.pcb.2[[congener_name]]
  ...
  covariate_columns <- names(grl.pcb.2)[!grepl("^PCB", names(grl.pcb.2))]
  
  # Select rows with non-missing values in the specified PCB congener and extract covariates
  congener_covariate_data <- grl.pcb.2[non_missing_indices, covariate_columns]
  ...
  hyperparameters <- expand.grid(
    num.trees = c(50, 100, 200),  # Example values, adjust as needed
    mtry = c(3, 4, min(5, ncol(train_data))),  # Ensure mtry is less than or equal to the number of variables
  ...
  results_rf_PCBi <- data.frame(
    Location = rep("Lake Michigan", length(test_indices)),
...
for (congener_name in colnames(grl.pcb.2)) {
