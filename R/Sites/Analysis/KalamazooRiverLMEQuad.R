...
lme.kal.tpcb <- lmer(log10(tpcb) ~ 1 + time + poly(flow, 2) + season + (1|site),
                  REML = FALSE,
...
# Look at residuals
{
  ...
  # Create pdf file
  pdf("Output/Plots/Sites/Q-Q/Q-QKalamazooRivertPCBQuad.pdf")
  qqnorm(res.kal.tpcb,
...
# Plot residuals vs. predictions
{
  ...
  png("Output/Plots/Sites/Residual/res_plotlmeKalamazooRivertPCBQuad.png",
...
# Create matrix to store results
{
  ...
  lme.tpcb[7] <- fixef(lme.kal.tpcb)[3] # flow linear
  lme.tpcb[8] <- summary(lme.kal.tpcb)$coef[3,"Std. Error"] # flow error
  lme.tpcb[9] <- summary(lme.kal.tpcb)$coef[3,"Pr(>|t|)"] # flow p-value
  lme.tpcb[10] <- fixef(lme.kal.tpcb)[4] # flow quadratic
  lme.tpcb[11] <- summary(lme.kal.tpcb)$coef[4,"Std. Error"] # flow error
  lme.tpcb[12] <- summary(lme.kal.tpcb)$coef[4,"Pr(>|t|)"] # flow p-value
