...
{
  # Change date format
  nbh$SampleDate <- as.Date(nbh$SampleDate, format = "%Y-%m-%d")
  # Calculate sampling time
  time.day <- data.frame(as.Date(nbh$SampleDate) - min(as.Date(nbh$SampleDate)))
  # Include season
  yq.s <- as.yearqtr(as.yearmon(nbh$SampleDate, "%Y-%m-%d") + 1/12)
...
# Look at residuals
{
  ...
  # Create pdf file
  pdf("Output/Plots/Sites/Q-Q/Q-QNewBedfordtPCB.pdf")
  qqnorm(res.nbh.tpcb,
...
# Plot residuals vs. predictions
{
  ...
  png("Output/Plots/Sites/Residual/res_plotlmeNewBedfordHarbortPCB.png", width = 800,
      height = 600)
  # Create plot
  plot(10^(predict(lme.nbh.tpcb)), resid(lme.nbh.tpcb),
       points(10^(predict(lme.nbh.tpcb)), resid(lme.nbh.tpcb), pch = 16, 
              col = "white"),
       ylim = c(-2, 2),
...
# Create matrix to store results from lme analysis
{
  ...
  # Calculate RMSE
  # Predictions
  predictions <- predict(lme.nbh.tpcb)
...
# Add Location Name
lme.tpcb <- cbind(LocationName = rep("New Bedford Harbor",
...
# Prepare data.frame
{
  nbh.pcb <- subset(nbh, select = -c(Source:AroclorCongener))
  # Remove Aroclor data
  ...
  # Change date format
  SampleDate <- as.Date(nbh$SampleDate, format = "%Y-%m-%d")
  # Calculate sampling time
...
for (i in 1:length(nbh.pcb.2[1,])) {
  ...
  # Calculate residuals and RMSE
  residuals <- nbh.pcb.2[, i] - predictions
  non_na_indices <- !is.na(residuals)
  lme.pcb[i, 13] <- sqrt(mean(residuals[non_na_indices]^2))
}
...
# Add factor of 2
# Create matrix to store results
lme.fit.pcb <- matrix(nrow = length(nbh.pcb.2[,1]),
                      ncol = length(nbh.pcb.2[1,]))

# Create a vector to store factor 2 for each congener
factor2_vector <- numeric(length = length(nbh.pcb.2[1,]))
...
for (i in 1:length(nbh.pcb.2[1,])) {
  ...
  factor2 <- 10^(lme.fit.pcb[, i])/10^(nbh.pcb.2[, i])
...
colnames(lme.pcb) <- c("Congeners", "Intercept", "Intercept.error",
                       ...
                       "R2nR", "R2R", "Normality", "RMSE", "Factor2")

# Add Location Name
lme.pcb <- cbind(LocationName = rep("New Bedford Harbor", nrow(lme.pcb)),
                 lme.pcb)
