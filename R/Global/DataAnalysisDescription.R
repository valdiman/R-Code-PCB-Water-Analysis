...
wdc.aroclorcongener <- wdc %>%
  mutate(Year = as.integer(format(as.Date(SampleDate, format = "%Y-%m-%d"), "%Y"))) %>%
...
{
  # Remove metadata
  wdc.nmd <- subset(wdc, select = -c(Source:AroclorCongener)) #nmd = no meta data
  # Remove Aroclor data
  wdc.nmd <- subset(wdc.nmd, select = -c(A1016:A1260))
  # (2) Only consider congener data
  wdc.cong <- subset(wdc, AroclorCongener == "Congener")
  # Remove metadata
  wdc.cong.1 <- subset(wdc.cong, select = -c(Source:AroclorCongener))
