...
server <- function(input, output, session) {
  ...
  wdc$date <- as.Date(wdc$SampleDate, format = "%m/%d/%y")
  ...
  filtered_data <- reactive({
    if (input$location_select == "All") {
      ...
      subset(wdc, LocationName == input$location_select)  # Filter by selected location
    }
  })
  ...
  output$data <- renderTable({
    if (!is.null(input$map_marker_click)) {
      ...
      filtered_data <- subset(wdc, SiteID == siteid)[, c("SiteID", "date", "tPCB")]
